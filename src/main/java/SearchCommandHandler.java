...
public class SearchCommandHandler implements ICommandHandler {
	...
	public boolean executeCommand() {
		...
		try {
			if (keyword != null) {
				...
				searchActualIds = this.taskData.searchEmptySlots(
						this.parsedDateStart, this.parsedDateEnd);
	 ...
	 * timings.
	 */
	public void processEmptySlots(List<Integer> searchActualIds, Date StartDate, Date EndDate) {
		String currDate;
		String nextDate;
		Event eventCurr;
		Event eventNext;
		...
		for (int i = 0; i < searchActualIds.size(); i++) {
			...
			if (eventCurr != null) {
				...
				for (int j = i + 1; j < searchActualIds.size(); j++) {
					...
					eventNext = taskData.getEventMap().get(
							searchActualIds.get(j));
					if (eventNext != null) {
	...
	// Once similar dates are found, their timings for each occupied task will be sorted in increasing order.
	public void sortEachDateTime(ArrayList<Event> commonDateIds) {
		...
		currTime = Integer.parseInt(formatHHmm.format(commonDateIds.get(0)
				.getTaskDate().getTime()));
		...
		for (limit = size - 2; limit >= 0; limit--) {
			for (i = 0; i <= limit; i++) {
				currTime = Integer.parseInt(formatHHmm.format(commonDateIds.get(i)
						.getTaskDate().getTime()));
				nextTime = Integer.parseInt(formatHHmm.format(commonDateIds
						.get(i + 1).getTaskDate().getTime()));
	...
	//Extract empty slot timings from a date and prints them in the display function
	public void extractEmptySlots(ArrayList<Event> commonDateIds) {
		...
		try {
			...
			this.initialDate = formatDayMthYr.parse(formatDayMthYr.format(commonDateIds.get(0).getTaskDate().getTime()));
			calStartTime.setTime(startTime);
			calEndTime.setTime(EndTime);
			System.out.println("Date: " + formatDayMthYr.format(initialDate));
			System.out.printf(messageEmptyText, "Empty From:", "To:","Task Occupied At:");
		} catch (ParseException e) {
			e.printStackTrace();
		}
		...
		for (Event event : commonDateIds) {
			...
			if (tomorrowDate != null && tomorrowDate.equals(formatDayMthYr.format(calCurrTime.getTime()))) {
			...
			if (!(startingTime.equals(currentTime))) {
				displayEmptyAndOccupiedResults(calStartTime, calEndTime, calCurrTime, event);
			}

			calCurrTime.add(Calendar.MINUTE, event.getTaskDuration());

			try {
				this.startTime = formatHHColonMM.parse(formatHHColonMM.format(calCurrTime.getTime()));
				this.finalDate = formatDayMthYr.parse(formatDayMthYr.format(calCurrTime.getTime()));
				calStartTime.setTime(startTime);
			} catch (ParseException e) {
				e.printStackTrace();
			}
		...
		if (finalDate.after(initialDate)) {
			this.excessDuration = calCurrTime.get(Calendar.HOUR) * 60 + calCurrTime.get(Calendar.MINUTE);
	...
	public void displayEmptyAndOccupiedResults(Calendar calStartTime,
		...
		if (event != null) {
			System.out.printf(messageEmptyDescriptions,
					...
					forDelimeter, Integer.toString(event.getTaskDuration()),
					minsDelimeter, event.getTaskName(), atDelimiter,
	...
	public void displaySearchResults(List<Integer> searchActualIds,
		...
		for (Integer searchId : searchActualIds) {
			...
			System.out.printf(messageDispResultsFormat,
					...
					formatTimeDayMthYr.format(event.getTaskDate().getTime()), atDelimiter,
					event.getTaskLocation(), event.getTaskDescription());
		}
	}
