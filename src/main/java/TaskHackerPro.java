...
public class TaskHackerPro {
    ...
    private static final String MESSAGE_WLECOME = "Welcome to TaskHackerPro!\nEnter \"help\" for system usage";
    ...
    private void showWelcomeMessage() {
        System.out.println(MESSAGE_WLECOME);
        ...
        for (Event overdueTask : overdueTasks) {
            ...
            Calendar dueTime = overdueTask.getTaskDueDate();
            String dueTimeString = dateFormat.format(dueTime.getTime());
            Color color;

            if (currentTime.before(overdueTask.getTaskDueDate())) {
                color = Color.GREEN;
            } else {
                color = Color.RED;
            }
            ...
            ConsoleUtility.printf(color, MESSAGE_OVERDUE_TASK_DISPLAY, displayId,
                    dueTimeString, overdueTask.getTaskName());
        }
