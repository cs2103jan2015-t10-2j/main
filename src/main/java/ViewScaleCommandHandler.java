...
public class ViewScaleCommandHandler implements ICommandHandler {
	...
	private static final Logger logger = Logger
			.getLogger("CalendarViewCommandHandler");
	...
	public boolean executeCommand() {
		...
		if (choseView == ViewOption.NOT_CHOSEN) {
			...
			Set<Integer> taskIds = getMatchingTaskDisplayIDs(dateViewing,
					choseView);
			switch (choseView) {
			case NOT_CHOSEN: {
				break;
				...
				for (Integer taskId : taskIds) {
					System.out.printf(
							...
							format.format(taskData.getEventMap().get(taskId)
									.getTaskDate().getTime()), taskData
									.getEventMap().get(taskId).getTaskName(),
							taskData.getEventMap().get(taskId)
									.getTaskLocation(), taskData.getEventMap()
									.get(taskId).getTaskDescription());
				}
			}
	...
	public Set<Integer> getMatchingTaskDisplayIDs(Calendar dateViewing,
		...
		for (Integer actualId : taskData.getEventMap().keySet()) {
			Calendar taskDate = taskData.getEventMap().get(actualId)
					.getTaskDate();
			...
			case THIS_WEEK: {
				isMatched = (taskDate.get(Calendar.WEEK_OF_YEAR) == dateViewing
						.get(Calendar.WEEK_OF_YEAR) && taskDate
			...
			case NEXT_MONTH: {
				isMatched = (taskDate.get(Calendar.MONTH) == (dateViewing
						.get(Calendar.MONTH) + 1) % 12 && taskDate
						.get(Calendar.YEAR) == dateViewing.get(Calendar.YEAR));
				break;
			}
			...
			case PREV_WEEK: {
				isMatched = (taskDate.get(Calendar.WEEK_OF_YEAR) == (dateViewing
						.get(Calendar.WEEK_OF_YEAR) - 1) && taskDate
						.get(Calendar.YEAR) == dateViewing.get(Calendar.YEAR));
				break;
			}
			...
			logger.info(String.format("actualId=%d, isMatched=%b", actualId,
					isMatched));
